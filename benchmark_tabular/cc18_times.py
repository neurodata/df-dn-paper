...
train_dict = train_test_times['train']
train_dict_tuple = {key:{key2:tuple(value) for key2,value in inner_res.items()} for key,inner_res in train_dict.items()}
# Save results as txt files
save_best_parameters(save_methods =save_methods,save_methods_rewrite = save_times_rewrite ,path_save = "results/times_train" ,best_parameters=  train_dict_tuple)

test_dict = train_test_times['test']
test_dict_tuple = {key:{key2:tuple(value) for key2,value in inner_res.items()} for key,inner_res in test_dict.items()}
# Save results as txt files
save_best_parameters(save_methods =save_methods,save_methods_rewrite = save_times_rewrite ,path_save = "results/times_test" ,best_parameters= test_dict_tuple)
