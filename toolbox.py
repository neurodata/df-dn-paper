...
def run_rf_image(
    ...
    for cls in classes:
        ...
        class_img = train_images[class_idx[:num_train_samples]]
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        with torch.no_grad():
            for i, data in enumerate(valid_loader, 0):
                ...
                loss = criterion(outputs, labels)
                cur_loss += loss
        #early stop if 3 epochs no loss decrease 
...
def create_loaders(train_labels, test_labels, classes, fraction_of_train_samples, trainset, testset, batch = 64):
...
def create_loaders_set(train_labels, test_labels, classes, trainset, testset, total_samples, batch = 64):
