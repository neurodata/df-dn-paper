...
def time_rf_cc18(SETUP_CODE, X_train_new, y_train_new, X_test, all_params, dataset):
    """
    Use timeit function to measure training and fitting time of the Random Forest classifier using optimized parameters for given dataset.
    """
    TEST_CODE = """
...
y_pred_rf = rf.predict(X_test)
    ...
    """.format(all_params,dataset)
    time_rf = timeit.repeat(setup=SETUP_CODE_RF, stmt=TEST_CODE, repeat=1, number=1, globals={'X_train_new':X_train_new,
...
y_pred_mlp = mlp.predict(X_test)
    ...
    """.format(all_params,dataset)
    time_mlp = timeit.repeat(setup=SETUP_CODE_MLP, stmt=TEST_CODE, repeat=1, number=1, globals={'X_train_new':X_train_new,
                                                                                        'y_train_new':y_train_new,
                                                                                        'X_test':X_test}
                        )
...
for dataset_index, dataset in enumerate(dataset_indices):
    ...
    for train_index, test_index in kf.split(X,y):
        ...
        for sample_size_index, max_sample_size in enumerate(training_sample_sizes):
            ...
            rf_time_temp = time_rf_cc18(SETUP_CODE_RF, X_train_new, y_train_new, X_test, all_params, dataset)
            mlp_time_temp = time_mlp_cc18(SETUP_CODE_MLP, X_train_new, y_train_new, X_test, all_params, dataset)
...
# np.savetxt('rf_evolution_times.txt', rf_times)
# np.savetxt('dn_evolution_times.txt', dn_times)     
