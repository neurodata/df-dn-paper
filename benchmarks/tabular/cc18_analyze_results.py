...
def plot_box(df_train, df_test, colors =['c','m'],ax= None, model_name = None, xlabel = 'Data Size'):
    ...
    model_name= model_name; ax.set_title('Train vs Test times %s'%model_name)
    ax.set_xlabel(xlabel);ax.set_ylabel('Running time [s]')
...
dictionary_train = {key:{key2:np.array(value) for key2,value in inner_res.items()} for key,inner_res in dictionary_train.items()}

dictionary_test = open_data(path_save_test,'text_dict')
dictionary_test = {key:{key2:np.array(value) for key2,value in inner_res.items()} for key,inner_res in dictionary_test.items()}
...
for model_num, model_name in enumerate(dictionary_train.keys()):
    ...
    plot_box(df_train, df_test, colors =available_colors[model_num],ax= axs1[model_num], model_name = model_name, xlabel = 'Data Size')
    plot_times_fill(df_train,available_colors[model_num][0],axs2[model_num],  'Train',model_name)
    plot_times_fill(df_test,available_colors[model_num][1],axs2[model_num], 'Test',model_name)    
    axs2[model_num].legend()
plt.figure(fig1); plt.savefig(figspath+'/times_box_plot.png');  plt.close(fig1)
fig2; plt.savefig(figspath+'/plot_times.png');  plt.close(fig2)
