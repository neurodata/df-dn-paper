...
for dataset_index, dataset in enumerate(train_indices):
    ...
    for sample_size_index, real_sample_size in enumerate(training_sample_sizes):
        ratio_base = [int(el) for el in np.linspace(0,real_sample_size, 4+1)]
        np.delete(ratio_base, 1)
        list_indices = np.arange(real_sample_size)
        np.random.seed(dataset_index)
        np.random.shuffle(list_indices)
        ss_inds_current = ss_inds[sample_size_index]
        ind_train = ss_inds_current[list_indices[0]:list_indices[1]]
        ind_val = ss_inds_current[list_indices[1]:list_indices[2]]
        ind_test = ss_inds_current[list_indices[2]:list_indices[3]]      
        
        X_train_new, X_val, X_test = X[ind_train], X[ind_val], X[ind_test]
        y_train_new, y_val, y_test = y[ind_train], y[ind_val], y[ind_test]
            ...
            if models_to_run[model_name]:
                ...
                for ii in range(reps): # ii 
                    try:
                        ...
                        cohen_kappa = cohen_kappa_score(y_val, predictions)
                        #cohen_kappa = cohen_kappa_score(y_test, predictions)
                        
                        cohen_ece_results_dict["cohen_kappa"][model_name][
                            ii
                        ] = cohen_kappa
                        ece = get_ece(predict_probas, predictions, y_val)
